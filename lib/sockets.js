...
class SocketServer {
  constructor(data) {
    ...
    else this.server = new WebSocket.Server({ server: data });
...
class Socket {
  ...
  on(eventName, action) {
    ...
    if (eventName == "connected" && this.readyState == 1) action();
...
class ServerSocket extends Socket {
  ...
  get isServer() {
    if (this.socket) return this.socket.isServer;
    else return false;
  }

  set isServer(val) {
    if (this.socket) this.socket.isServer = val;
  }
}
...
class ClientSocket extends Socket {
  ...
  configureOpen() {
    this.socket.on("open", () => {
      console.log(`Socket connection to server opened: ${this.path}`);
    });
  }

  configureClose() {
    this.socket.on("close", () => {
      console.log(`Socket connection to server closed: ${this.path}, retrying in ${this.reconnectBackoff / 1000} seconds`);
      if (this.persistent) {
        setTimeout(() => {
          this.reconnect();
        }, this.reconnectBackoff);
        this.reconnectBackoff = Math.min(this.reconnectBackoff + 1000, 10000);
      }
    });
  }
...
class TestSocket {
  ...
  onClientEvent(eventName, action) {
    if (!this.clientListeners[eventName]) this.clientListeners[eventName] = [];
