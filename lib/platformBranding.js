...
function getCarouselBannerRelativePath(bannerId) {
  return `branding/banners/carousel/${bannerId}.webp`;
}
...
function buildBrandingPayload(doc) {
  ...
  if (doc?.carouselBanners && Array.isArray(doc.carouselBanners)) {
    for (const item of doc.carouselBanners) {
      carousel.push({
        id: item._id,
        _id: item._id,
        url: toCacheablePublicUrl(item.path, updatedAt),
      });
    }
  }
